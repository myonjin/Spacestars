...
export async function requestAlarmApprove(
  token: string,
  bodyData: AlarmRequestType,
) {
  try {
...
// 친구요청 거절
export async function requestAlarmReject(
  token: string,
  bodyData: AlarmRequestType,
) {
  try {
...
// 매칭요청 수락
export async function requestMatchingApprove(
  token: string,
  ...
  try {
    ...
    const response = await fetch(
      `${MATCHING_BASE_URL}/agree?fromMemberUuid=${fromMemberUuid}`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
...
// 매칭요청 거절
export async function requestMatchingReject(
  token: string,
  fromMemberUuid: string,
) {
  try {
    ...
    const response = await fetch(
      `${MATCHING_BASE_URL}/reject?fromMemberUuid=${fromMemberUuid}`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
